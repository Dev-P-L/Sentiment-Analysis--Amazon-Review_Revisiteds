...
# Keeping only the second column; 
# removing leading and trailing empty space characters;
# vectorizing the resulting text data.

short_forms_pos <- 
  str_trim(short_forms_pos[, 2]) %>% 
  as.vector()
...
for (i in 1:nrow(reviews_training)) {
  ...
  for (j in 1:length(super_positive_unigrams)) {
    ...
    for (k in 1:length(negation)) {
      dummy <- paste(negation[k], 
                     super_positive_unigrams[j], 
                     sep = "")
