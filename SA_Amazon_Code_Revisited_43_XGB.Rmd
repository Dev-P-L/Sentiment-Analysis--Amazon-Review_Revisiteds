...
IDs <- c("cart_15", "xgbLinear_5")

models <- c("CART", "eXtreme Gradient Boosting")

caret_names <- c("rpart", "xgbLinear")
...
df <- train[, 1:(ncol(train) - 1)]

# Any combination of a token (column) and a training review (row) from the data frame "train" is a real number at least equal to zero. This number indicates the occurrences frequency of the token in the training review. But, in a first step, we are only interested in discarding with no or little variation. And we'll do it in this way.

# All numbers greater than zero will be set equal to 1.
df[df > 0] <- 1

# Columns will be summed up to indicate in how many training reviews a token is present (once, or twice, thrice ... it doesn't matter).
v <- colSums(df)

# Only columns summing up to more than 6 will be kept.  
l <- which(v > 0)

# Adding the number of the last column of train, i.e. the dependent variable with sentiment polarity. 
l <- append(l, ncol(train))

# Discarding low variation columns from the data frame "train".
train <- train[ , l] %>% 
  as.data.frame()
...
pred_xgbLinear_5 <- predict(fits$xgbLinear_5, newdata = val)
pred_cart_15 <- predict(fits$cart_15, newdata = val)

# Accuracy on the validation set with xgb (and CART)

acc_xgbLinear_5 <- round(mean(pred_xgbLinear_5 == val$sentiment), 4)
...
tab <- data.frame(matrix(c(acc_baseline, acc_cart_15, acc_xgbLinear_5), 
                         nrow = 3, ncol = 1)) %>%
...
rm(fits)
rm(pred_xgbLinear_5, pred_baseline, pred_cart_15, ref, tab)
```
