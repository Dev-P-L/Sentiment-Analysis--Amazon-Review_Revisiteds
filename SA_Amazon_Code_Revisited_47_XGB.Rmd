...
# SA_Amazon_Insights&Results_Revisited.html. Anyway, 
# messages and warnings produced by the code 
...
**90 % prediction accuracy** has been reached in this project predicting sentiment polarity on Amazon reviews. Classification is binary, with positive and negative polarity. There are 1,000 reviews in the data set, from the UCI Machine Learning Repository. The training set is a random sample containing two thirds of the data set. The validation set is used only at the very end, to validate the model. The final model is delivered by the algorithm eXtreme Gradient Boosting Tree; CART is used in the traing process because it is swift and delivers clear outputs. 

The 90 % prediction accuracy has been reached on the validation set against 50 % with a baseline model. Natural Language Processing, Text Mining and Machine Learning optimization have all contributed the accuracy performance. 

**Natural Language Processing** was about corpus, lowercasing, punctuation handling, stopword removal, stemming, tokenization from sentences into words, bag of words. 

**Text mining** has brought insights about subjective information. In CART decision trees predominate some tokens conveying subjective information; but other tokens containing subjective information have not been used in false negatives and false positives. Such ignored subjective information has been retrieved from false negatives and false positives, exclusively on the training set; customized lists have been established with tokens sorted as having either positive or negative sentiment orientation; occurrences of these tokens in reviews have been replaced with either a positive or a negative generic token. 

Text Mining has also brought another insight, quantitatively much less impactful though: negation has been fruitfully integrated.  

**Machine Learning optimization** had been performed across ten models in a previous version of this project, with testing has been conducted on accuracy distributions across bootstrapped resamples. See https://github.com/Dev-P-L/Sentiment-Analysis--Amazon-Reviews .

**eXtreme Gradient Boosting** had emerged as the most performing model in this project. This conclusion has been overtaken in the current version without testing again all other models. It has boosted accuracy with 5 percentage points. 

This version has been realized to introduce **interactive tables and figures**, which have greatly facilitated text mining. It has also been an opportunity to use **XGBoost Tree** instead of XGBoost Linear, further to results reported in data science literature.
...
It is comprised of twelve files. All code is included in SA_Amazon_Code_Revisited.Rmd. It does not show in the result report, called SA_Amazon_Insights&Results_Revisited.html. 
...
On my standard laptop, knitting SA_Amazon_Code_Revisited.Rmd takes approximately one and a half hours. For information about my work environment, see the session info at the end of this document. 
...
Consequently, we cannot apply the baseline model in prediction. Indeed, considering that all reviews have e.g. positive polarity would deliver 50 % true positives and 50 % false positives. 
...
- to produce a bag of words or document matrix;
...
We are going to treat these misspelled short forms as if they were standardly written. We will complement stopwords with variants such as "dont", "couldnt". Consequently, when we remove stopwords, the misspelled short forms can be eradicated as well as the standardly written short forms, at least for the misspelled short forms we can think of... Complementing stopwords with misspelled short forms will be done in the next section "Fine Tuning NLP".
...
# Localizing the cases, which originated 
# in misspelling. These are all cases except 
...
# the row number(s) generating "brokeni".
...
# and preparing them in order to rebuild the corpus, 
# the bag of words (the Document Term  Matrix) 
# and the wordcloud. 
...
# short forms such as "don't" and be able to identify them 
# as short forms and as such to discard them. 
...
cm_cart_av0 <- 
  confusionMatrix(fitted_cart_av0, sentSparse_av0$sentiment)
...
# Running rpart on the training set with train() from caret. 
...
Accuracy increases from 77 to 78. For the record, let's have a look at a graph showing how accuracy evolves across the 15 cp values chosen by the train() function. 
...
The optimal value of cp is zero. This means that the train() function has kept the decision tree as complex as possible by assigning a zero value to the complexity parameter. This might indicate that accuracy improvement could come from more complex models. This will be done later on.

On the graph above, maximum accuracy is lower than the level previously indicated. Why is it different? Because, on the graph, it is, for each cp value, the average accuracy on the 25 bootstrapped resamples, while accuracy previously given related to the whole training set. 

On the whole training set, the rpart model without tuning delivers approximately 77 % accuracy and the rpart model with tuning 78 %. Both levels are substantially higher than accuracy provided by the baseline model. 
...
pred_baseline <- 
  ...
  mutate(sentiment = 
           factor(sentiment, levels = levels(df$sentiment)))
...
There is a majority of tokens conveying subjective information, such as *great*, *comfort*, *love*, *like*, *disappoi*, even if *like* can be ambiguous because it can be the preposition and not the verb. 
...
# Building up a vector with the 40 most frequent tokens 
# in the bag of words.
...
Most decision tree tokens appear in the wordcloud (or the histogram). To check it up, let's compute the proportion of decision tree tokens appearing in the wordcloud or in the histogram. 
...
# appearing in the wordcloud or in the histogram.
...
# class.output = "bg-primary" gives white color 
# and blue background color.
...
Acting on the first reason could be done by choosing a more effective algorithm than *rpart*. This will not be done at this stage because a more effective algorithm can have a tendency to stick to data, even to outliers, on the training set and to be somewhat disappointing on the validation set (overfitting). This might camouflage problems. 
...
In this project, polarity of some tokens conveying positive subjective information will be inserted in additional files. That is one avenue of improvement that will be investigated in the section *Predicting after TM* (Text Mining). 
...
# Let's determine the patterns we are looking for 
# in false positives. Since most comments are capitalized, 
# variants have been provided with capitalization. 
# Around some negational unigrams, there is a leading 
# empty space character and/or a trailing empty space 
# character in order to avoid picking up longer unigrams 
# containing some negational unigrams, e.g. "notice" 
# instead of "no" or instead of "not". 
...
# A data frame with the false positives has already 
# been created. It has been called df_fp. Column names 
# are "Row", "Training Review", and "Tokenized". 
# Let's keep only rows with at least one of the words 
...
# Let's determine the patterns we are looking for 
# in false negatives. Since most comments are capitalized, 
# variants have been provided with capitalization. 
# Around some negational unigrams, there is a leading 
# empty space character and/or a trailing empty space 
# character in order to avoid picking up longer unigrams 
...
# A data frame with the false negatives has already been 
# created. It has been called df_fn. Column names are "Row", 
...
This difference between wordcloud and decision tree might be insightful. Should topic-related tokens be discarded except the few ones that appear in the decision tree? Should they be excluded to make the bag of words less messy? But would they systematically be excluded as well in another machine learning model? In a previous version of this project, this has been tested and it appeared that topic-related tokens seemed more impactful in another machine learning model, which was *random forests*. For safety, they will all be kept. 
...
# Same process for super positive unigrams 
...
# characters except for apostrophes. 

for (i in 1:nrow(reviews_training)) {
  corpus_av2[[i]]$content <- 
    gsub("(?!')[[:punct:]]", " ", 
      corpus_av2[[i]]$content, perl = TRUE)
}

# Removing spaces at the beginning and at the end 
# of reviews to get apostrophes in first or last position
# if they are at the beginning or at the end of a review.
...
# Removing apostrophes at the beginning 
...
# generic polarized unigram " subjpo " or " subneg ".
...
# Replacing all remaining apostrophes 
...
# Polarizing multigrams again (some apostrophes 
...
# and at the end of each review. Consequently, 
...
for (i in 1:nrow(reviews_training)) {
  for (j in 1:length(short_forms_neg)) {
    corpus_av2[[i]]$content <- 
      gsub(paste(" ", short_forms_neg[j], " subjneg ", collapse = ""), 
           " subjpo ", corpus_av2[[i]]$content)
  }
}
...
for (i in 1:nrow(reviews_training)) {
  for (j in 1:length(short_forms_neg)) {
    corpus_av2[[i]]$content <- 
      gsub(paste(" ", short_forms_neg[j], " subjpo ", collapse = ""), 
           " subjneg ", corpus_av2[[i]]$content)
  }
}

# Flipping polarity of any combination of
# negation + super positive unigram.
...
# Flipping polarity of any combination of 
# negative short form + super positive unigram.

for (i in 1:nrow(reviews_training)) {
  for (j in 1:length(super_positive_unigrams)) {
    for (k in 1:length(short_forms_neg)) {
      dummy <- paste(short_forms_neg[k], super_positive_unigrams[j], 
                     collapse = "")
      dummy <- paste("", str_squish(dummy), "")
      corpus_av2[[i]]$content <-  
        gsub(dummy, " subjneg ", corpus_av2[[i]]$content)
...
# Removing numbers, digits and extra empty space characters.
...
Polarizing n-grams that convey subjective information has dramatically boosted accuracy from approximately 78 % to 91 %. Nevertheless, let us stay realistic: text mining has been performed on the training set and is not necessarily, or at least not necessarily entirely, transposable to the validation set. 
